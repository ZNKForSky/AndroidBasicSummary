...
public class FadeNotificationService extends Service {
    ...
    public static boolean trigger(Context context) {
        ...
        if (!start) {
            context.startService(new Intent(context, FadeNotificationService.class));
        } else {
            if (Build.VERSION.SDK_INT < 18) {
                context.stopService(new Intent(context, FadeNotificationService.class));
            } else {
